...
class AzureTransliterator:
    ...
    def transliterate(
        ...
        try:
            ...
            logging.info(
                f"Azure Transliteration: {language} {from_script}->{to_script}: {text}"
            )
            ...
            if response.status_code == 200:
                ...
                if result and len(result) > 0 and "text" in result[0]:
                    ...
                    logging.error(
                        f"Azure Transliteration: Unexpected response format: {result}"
                    )
                    return None
            else:
                logging.error(
                    f"Azure Transliteration API error: {response.status_code} - {response.text}"
                )
                return None
    ...
    def get_supported_languages(self) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.error(
                    f"Azure Languages API error: {response.status_code} - {response.text}"
                )
                return None
...
def transliterate_text(
    ...
    try:
        ...
        if not subscription_key or not region:
            logging.error(
                "Azure transliteration credentials not found in [transliterate] or [translate] configuration sections"
            )
            return None
        ...
        if not all([language, from_script, to_script]):
            logging.error(
                "Transliteration settings incomplete: language, from_script, and to_script are required"
            )
            return None
