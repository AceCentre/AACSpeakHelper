...
# Log file path (adjust if CreateGridset.exe's path differs)
log_path = os.path.join(os.environ.get('APPDATA', ''), 'Ace Centre', 'AACSpeakHelper', 'creategridset_log.txt')
...
def modify_gridset(gridset_path, LocalAppPath):
    try:
        ...
        for foldername, _, filenames in os.walk(temp_dir):
            for filename in filenames:
                if filename.endswith('.xml'):
                    ...
                    full_path_to_exe = os.path.join(local_app_data_path, 'Programs', 'Ace Centre', 'AACSpeakHelper', 'client.exe')
        ...
        write_log(f"Removed original gridset file: {gridset_path}")
...
if __name__ == "__main__":
    try:
        ...
        gridset_location = os.path.join(app_data_path, 'Ace Centre', 'AACSpeakHelper', 'TranslateAndTTS DemoGridset.gridset')
