...
class ConfigMigrator:
    ...
    def migrate_config(self, config: configparser.ConfigParser) -> configparser.ConfigParser:
        """
        ...
        for (old_section, old_key), (new_section, new_key, transform) in self.MIGRATION_MAP.items():
            if config.has_option(old_section, old_key):
                ...
                self.logger.info(f"Migrated {old_section}.{old_key}={old_value} -> {new_section}.{new_key}={new_value}")
    ...
    def get_boolean_with_fallback(self, config: configparser.ConfigParser,
                                 section: str, key: str, fallback: bool = False) -> bool:
        """
        Get boolean value - new positive format only.
        """
        return config.getboolean(section, key, fallback=fallback)
    
    def get_string_with_fallback(self, config: configparser.ConfigParser,
                                section: str, key: str, fallback: str = "") -> str:
        """
